...
function balanceIndex(nums) {
  for (let i = 0; i < nums.length; i++) {
    ...
    for (let j = 0; j < i; j++) {
      //we start this loop at 0 because we want to include the index itself (j i K) during the first iteration the left side will be 0
      leftSum += nums[j]; // left side of index
    }
    for (let k = i + 1; k < nums.length; k++) {
      // we start at i + 1 because we don't want to include the index itself
      // right side of index
