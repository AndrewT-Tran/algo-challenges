...
function longestPalindromicSubstring(str) {
  ...
  for (var i = 0; i < str.length - 1; i++) {
    // for loop to iterate forward through string
    for (var j = str.length - 1; j > i; j--) {
      // for loop to iterate backwards through string, stops when it reaches i
      if (str[i] == str[j]) {
        // if value at i matches j
        temp = str.slice(i, j + 1); // split string from i to j (+1 to include j), store in temp
        if (isPalindrome(temp)) {
          // put temp through isPalindrome check
